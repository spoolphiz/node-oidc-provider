...
class Provider extends events.EventEmitter {
  ...
  async interactionFinished(req, res, result) {
    ...
    // Alex Manea: this is a hack to force the return url
    // to use https instead of http, there should be a better
    // way to do this...
    const parsedReturnTo = new URL(returnTo);
    parsedReturnTo.protocol = 'https';

    res.statusCode = 302; // eslint-disable-line no-param-reassign
    res.setHeader('Location', parsedReturnTo.toString());
